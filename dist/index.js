...
function bundleAndDeploy() {
  try {
    ...
    if (!config?.lynxProject) {
      ...
      lynxProject = path4.join(process.cwd(), config?.lynxProject);
  } catch (error) {
    console.error(`❌ Error loading configuration: ${error.message}`);
    process.exit(1);
  }
...
class LynxProvider: NSObject, LynxTemplateProvider {
  func loadTemplate(withUrl url: String!, onComplete callback: LynxTemplateLoadBlock!) {
	...
	`);
  writeFile(path6.join(projectDir, "LynxInitProcessor.swift"), `
...
`;
    ...
    fs7.writeFileSync(filePath, fileContent, "utf8");
  ...
  function updateLynxInitProcessor(packages) {
    let appNameFromConfig;
    try {
      const cfgPath = path7.join(process.cwd(), "tamer.config.json");
      if (fs7.existsSync(cfgPath)) {
        const cfgRaw = fs7.readFileSync(cfgPath, "utf8");
    ...
    if (appNameFromConfig) {
      candidatePaths.push(path7.join(iosProjectPath, appNameFromConfig, "LynxInitProcessor.swift"));
    }
    candidatePaths.push(path7.join(iosProjectPath, "LynxInitProcessor.swift"));
    const found = candidatePaths.find((p) => fs7.existsSync(p));
    const lynxInitPath = found ?? candidatePaths[0];
    const iosPackages = packages.filter((p) => p.config.ios && p.config.ios.moduleClassName);
...
${fileContent}`;
      ...
      fs7.writeFileSync(filePath, newContent, "utf8");
      console.log(`✅ Updated imports in ${path7.basename(filePath)}`);
    }
...
`);
  ...
  function run() {
    ...
    if (appNameFromConfig) {
      const appPodfile = path7.join(iosProjectPath, appNameFromConfig, "Podfile");
      if (fs7.existsSync(appPodfile)) {
...
function bundleAndDeploy2() {
  ...
  try {
    ...
    const config = JSON.parse(configRaw);
    lynxProject2 = config.lynxProject ? path8.join(process.cwd(), config.lynxProject) : process.cwd();
    appName = config.ios?.appName;
    if (!appName) {
  ...
  try {
    console.log("\uD83D\uDCE6 Starting the build process...");
    execSync5("npm run build", { stdio: "inherit", cwd: lynxProject2 });
    console.log("✅ Build completed successfully.");
  } catch (error) {
    console.error("❌ Build process failed. Please check the errors above.");
    process.exit(1);
...
`);
  ...
  const lynxProject2 = await ask("Lynx project path (relative to project root, e.g. packages/example) [optional]: ");
  const config = {
    android: {
  ...
  if (lynxProject2) {
    config.lynxProject = lynxProject2;
  }
...
program.command("autolink").description("Auto-link native modules to the project").action(async () => {
  const configPath = path10.join(process.cwd(), "tamer.config.json");
  let config = {};
  if (fs10.existsSync(configPath)) {
    config = JSON.parse(fs10.readFileSync(configPath, "utf8"));
  }
  if (config.autolink) {
